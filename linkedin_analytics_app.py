...
app_dash = dash.Dash(__name__, server=server, external_stylesheets=external_stylesheets, url_base_pathname='/dash/')
...
def receiver():
    username_string = request.form.get('username')
    password_string = request.form.get('password')
    link_username = request.form.get('link_username')
    number_of_posts = int(request.form.get('number_of_posts'))

    # change path to the path of your chromdriver on your local machine
    ...
    likes_bs4tags = soup.find_all("span",
